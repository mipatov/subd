...
class HelpBrowser(QWebEngineView):
    ...
    def setUU(self, url):
        cont = self.helpEngine.fileData(url)
        self.setHtml("doc/doc.htm")
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def createHelpWindow(self):
        self.helpEngine = QtHelp.QHelpEngine(
            os.path.join(CURRENT_DIR, "doc", "help.qhc")
        )
        self.helpEngine.setupData()
        # lll = self.helpEngine.fileData()
        # print(lll)
        ...
        self.ww = self.helpEngine.contentWidget()
        self.ww.show()

        self.helpEngine.setUsesFilterEngine(True)
        self.helpEngine.contentWidget().linkActivated.connect(textViewer.setUU)
        self.helpEngine.indexWidget().linkActivated.connect(textViewer.setUU)
